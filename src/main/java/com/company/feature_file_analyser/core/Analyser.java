...
public class Analyser extends FilesReader {
    ...
    public void calculateCodeReuseAtAcceptanceTestLevel() {
        readFeatureFileAndStepProperties();
        log.info("Keywords and Parameters read-in across all Feature Files");
        readScenariosAndOutlines();
        log.info("Data Table Row Counts read-in across all Feature Files");
        analyseData();
...
//                step.setDataTableDriven(isDataTableDriven);
    ...
    private void calculateImpactOfBackgroundKeyword() {
        ...
        long count = getListOfAllSteps().stream().distinct()
...
//                count += step.getFilePathDataTableDrivenCountForStep();
    ...
    private int sumDataTableDrivenRowCountAcrossFilesForSingleParameterisedStep(String stepName) {
        List<Step> filteredObjList = getListOfAllSteps().stream()
...
//                count += step.getFilePathDataTableDrivenCountForStep();
    ...
    private long countAllStepDataDrivenRecurrences() {
        return getListOfAllSteps().stream()
                .filter(Step::isDataDriven)
                .count();
    }
    ...
    private long countStepRecurrencesWithoutReuse(String stepName) {
        long count = getListOfAllSteps().stream()
                .filter(s -> s.getStepName().equalsIgnoreCase(stepName))
                .count();
    ...
    private long countTotalNumberOfScenarioRecurrences() {
        List<Step> filteredList = getListOfAllSteps().stream()
